...
You run everything from your own computer. The tool reads a simple **cut sheet** (`targets.yml`), connects to each VM **one at a time**, performs safe updates and health checks, and then gives you a clear **OK vs FAILED** summary.
...
1. You define your VMs in `targets.yml`
2. You run `./run`
3. Each VM is processed **sequentially**
4. Results are summarized
5. You choose what to do next from a menu
...
- Adds the VM to your `known_hosts` file  
...
- Useful to confirm services like Jellyfin or Nextcloud are reachable
